...
async def enqueue_profile_command(action: str, actor: dict, payload: dict) -> dict:
    command_id = str(uuid.uuid4())
    envelope = build_envelope(
        event_type=topics.PROFILE_COMMANDS,
        actor_id=str(actor["user_id"]),
        entity_type="profile_command",
        ...
        idempotency_key=f"profile.command:{command_id}",
        schema_ref="com.skillsync.profile.command.v1",
    )
    await set_command_status(command_id, {"command_id": command_id, "status": "queued", "action": action})
    await publish_event(topics.PROFILE_COMMANDS, envelope)
