...
async def enqueue_connection_command(action: str, actor: dict, payload: dict) -> dict:
    command_id = str(uuid.uuid4())
    envelope = build_envelope(
        event_type=topics.CONNECTION_COMMANDS,
        actor_id=str(actor["user_id"]),
        entity_type="connection_command",
        ...
        idempotency_key=f"connection.command:{command_id}",
        schema_ref="com.skillsync.connection.command.v1",
    )
    await set_command_status(
        command_id,
        {"command_id": command_id, "status": "queued", "action": action},
    )
    await publish_event(topics.CONNECTION_COMMANDS, envelope)
