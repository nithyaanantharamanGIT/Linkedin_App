...
from consumers.recruiter_commands_consumer import start_recruiter_commands_consumer
from routes.recruiters import router
...
async def lifespan(app: FastAPI):
    await get_pool()
    await get_redis()
    await start_recruiter_commands_consumer()
    yield
    await close_pool()
    await close_producer()


app = FastAPI(title="Recruiter Service", lifespan=lifespan)
app.add_middleware(LoggerMiddleware)
app.add_middleware(TraceMiddleware)
register_error_handlers(app)
app.include_router(router, prefix="/recruiters")


@app.get("/health")
async def health():
    return {"success": True, "data": {"service": "recruiter-service", "status": "ok"}}
