...
async def enqueue_recruiter_command(action: str, actor: dict, payload: dict) -> dict:
    command_id = str(uuid.uuid4())
    envelope = build_envelope(
        event_type=topics.RECRUITER_COMMANDS,
        actor_id=str(actor["user_id"]),
        entity_type="recruiter_command",
        ...
        idempotency_key=f"recruiter.command:{command_id}",
        schema_ref="com.skillsync.recruiter.command.v1",
    )
    await set_command_status(command_id, {"command_id": command_id, "status": "queued", "action": action})
    await publish_event(topics.RECRUITER_COMMANDS, envelope)
